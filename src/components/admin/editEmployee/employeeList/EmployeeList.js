...
const EmployeeList = ({
  ...
  const fetchEmployee = async () => {
    ...
    try {
      ...
      setFilteredEmployee(
        filteredData.filter((employee) => employee.gender === gender)
      );
    } catch (error) {
  ...
  const groupedEmployee = employee.reduce((acc, employee) => {
    const { position, gender } = employee;
    if (!acc[gender]) acc[gender] = {};
    if (!acc[gender][position]) acc[gender][position] = [];
    acc[gender][position].push(employee);
    return acc;
  }, {});
  ...
  return (
    <div className={styles["employee-list"]}>
      ...
      {Object.keys(groupedEmployee).map((genderKey) => (
        <div key={genderKey}>
          <h3 className={styles.gender}>
            Пол:
            {genderKey == 1
              ? "Мастер по Мужским услугам"
              : "Мастер по женским услугам"}
          </h3>
          {Object.keys(groupedEmployee[genderKey]).map((position) => (
            <div key={position}>
              <h4 className={styles.category}>{getPositionText(position)}</h4>
              <ul className={styles["employee-list__inner"]}>
                {groupedEmployee[genderKey][position].map((employee, index) => (
                  <li className={styles["employee-item"]} key={index}>
                    {EmployeeId === employee.id ? (
                      ...
                      <>
                        ...
                        <div className={styles["employee-item__inner"]}>
                          <strong>Фамилия Сотрудника:</strong>
                          <p>{employee.lastName}</p>
                        </div>
                        <div className={styles["employee-item__inner"]}>
                          <strong>Логин:</strong> <p>{employee.login}</p>
                        </div>
                        <div className={styles["employee-item__inner"]}>
                          <strong>Пароль:</strong> <p>{employee.password}</p>
                        </div>
                        <div className={styles["employee-item__inner"]}>
                          <strong>Email:</strong> <p>{employee.email}</p>
                        </div>
                        <div className={styles["employee-item__inner"]}>
                          <strong>Телефон:</strong> <p>{employee.phone}</p>
                        </div>
                        <div className={styles["employee-item__inner"]}>
                          <strong>Дата трудоустройства:</strong>
                          <p>{formatDate(employee.dateWorkIn)}</p>
                        </div>

                        <div className={styles["employee-item__inner"]}>
                          <strong>Пол:</strong>{" "}
                          <p>{getGenderText(employee.gender)}</p>
                        </div>
                        <div className={styles["button-block"]}>
                          ...
                          <CustomButton
                            ...
                            onClick={() =>
                              showMessageDeleteEmployee(employee.id)
                            }
                          />
                              ...
                              <div className={styles["modal-overlay"]}>
                                <div className={styles["modal-content"]}>
                                  ...
                                  <div className={styles["btn-block"]}>
                                    ...
                                    <CustomButton
                                      className={
                                        styles["cancel-delete__employee"]
                                      }
                                      type="button"
